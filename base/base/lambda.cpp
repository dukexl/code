...
函数对象参数是传递给编译器自动生成的函数对象类的构造函数的。函数对象参数只能使用那些到定义Lambda 为止时 Lambda 所在作用范围内可见的局部变量(包括 Lambda 所在类
的 this)。函数对象参数有以下形式：
...
4) [this] this。函数体内可以使用 Lambda 所在类中的成员变量。
5) [a] a。将 a 按值进行传递。按值进行传递时，函数体内不能修改传递进来的 a 的拷贝，因为默认情况下函数是 const 的，要修改传递进来的拷贝，可以添加 mutable 修饰符。
6) [&a] &a。将 a 按引用进行传递。
7) [a,&b] a，&b。将 a 按值传递，b 按引用进行传递。
8) [=，&a，&b] =，&a，&b。除 a 和 b 按引用进行传递外，其他参数都按值进行传递。
9) [&，a，b] &，a，b。除 a 和 b 按值进行传递外，其他参数都按引用进行传递。
