...
void KafkaProducer::Send(const string &key, const string &msg)
{
    if (!m_bRun)
        return;
...
int main()
{
    KafkaProducer *Kafkapr_ = new KafkaProducer("120.78.227.40:9092", "AQuote", 0);
    Kafkapr_->Init();
    Kafkapr_->Send("xuli", "hello world!");
